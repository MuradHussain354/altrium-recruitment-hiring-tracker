import React from 'react';
import { useAuth } from '../auth/AuthContext';
import { ShieldCheck, Users, BarChart3, ChevronRight } from 'lucide-react';

export const ManagerHomePage: React.FC = () => {
  const { user } = useAuth();

  return (
    <div>
      {/* Banner */}
      <div className="glass-card" style={{ padding: '28px 32px', marginBottom: '28px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
          <span className="badge badge-indigo">
            <ShieldCheck size={14} /> Manager Executive Portal
          </span>
        </div>
        <h1 style={{ fontSize: '1.85rem', fontWeight: 800, color: 'var(--text-main)', marginBottom: '8px' }}>
          Welcome back, {user?.name}!
        </h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', margin: 0 }}>
          Manage user accounts, assign team roles, and oversee recruitment pipeline reporting analytics.
        </p>
      </div>

      {/* Feature Preview Cards */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
        <div className="glass-card" style={{ padding: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
            <Users color="var(--primary)" size={22} />
            <h3 style={{ fontSize: '1.15rem', fontWeight: 700 }}>Account & Team Management</h3>
          </div>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginBottom: '16px' }}>
            Provision HR and Team Lead user accounts, manage team memberships, and toggle account activation status.
          </p>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--primary)', fontSize: '0.85rem', fontWeight: 600 }}>
            <span>Module Scheduled for Batch 2</span>
            <ChevronRight size={14} />
          </div>
        </div>

        <div className="glass-card" style={{ padding: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
            <BarChart3 color="var(--accent-cyan)" size={22} />
            <h3 style={{ fontSize: '1.15rem', fontWeight: 700 }}>Manager Reporting Analytics</h3>
          </div>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginBottom: '16px' }}>
            Monitor recruitment pipeline velocity, candidate funnel metrics, conversion ratios, and interviewer workloads.
          </p>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--accent-cyan)', fontSize: '0.85rem', fontWeight: 600 }}>
            <span>Module Scheduled for Batch 2</span>
            <ChevronRight size={14} />
          </div>
        </div>
      </div>
    </div>
  );
};

export default ManagerHomePage;
